...
class Search {
  ...
  getResults() {
    jquery__WEBPACK_IMPORTED_MODULE_0___default().when(jquery__WEBPACK_IMPORTED_MODULE_0___default().getJSON(universityData.root_url + '/wp-json/wp/v2/pages?search=' + this.searchField.val()), jquery__WEBPACK_IMPORTED_MODULE_0___default().getJSON(universityData.root_url + '/wp-json/wp/v2/posts?search=' + this.searchField.val())).then((pages, posts) => {
      var combinedResults = pages[0].concat(posts[0]);
      this.resultsDiv.html(`
                <h2 class="search-overlay__section-title">General Information</h2>
                ${combinedResults.length ? '<ul class="link-list min-list">' : '<p>No information avaialable with your matching query</p>'}
                    ${combinedResults.map(each => `<li><a href="${each.link}">${each.title.rendered}</a></li>`).join('')}
